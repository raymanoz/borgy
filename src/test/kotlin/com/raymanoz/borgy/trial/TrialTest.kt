...
class TrialTest {
    ...
    fun `select next increments selected observation`() {
        val trial = Trial("foo", listOf(Observation("hotness", emptyList()), Observation("coldness", emptyList())), 0)
        assertThat(trial.selectNext().selectedObservation, equalTo(1))
    }

    @Test
    fun `select next does not increment past last observation`() {
        val trial = Trial("foo", listOf(Observation("hotness", emptyList()), Observation("coldness", emptyList())), 1)
        assertThat(trial.selectNext().selectedObservation, equalTo(1))
    }
